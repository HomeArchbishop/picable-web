<template>
  <router-link :to="link" custom v-slot="{ navigate, href }">
    <a class="recommend-item" @click="navigate" :href="href">
      <div class="main-area">
        <div class="img-div" @click.stop.prevent="$utils.viewLargeImage($utils.formatImgUrl(item.thumb.fileServer, item.thumb.path))">
        ...
        <div class="content-div">
          ...
          <div class="tag-div">
            <router-link  v-for="tagName in item.tags" :key="tagName"
              :to="{ name: 'SearchTag', query: { t: tagName } }" custom v-slot="{ navigate, href }"
            >
      ...
      <div class="tip-area heavy" v-if="isHeavyTaste" @click.stop.prevent>重口</div>
      <div class="tip-area no-h" v-if="isNoH" @click.stop.prevent>无H内容</div>
      <div class="tip-area rare" v-if="isRare" @click.stop.prevent>生肉</div>
    </a>
  </router-link>
</template>
