    ...
    async judgeHasNextEpisodes () {
      ...
      try {
        while (!episodesObject || episodesObject.page < episodesObject.pages) {
          ...
          let findTitleFlag = false
          let findNextFlag = false
          for (const { order, title } of (episodesObject.docs || [])) {
            if (order === +this.epsOrder) {
              this.$store.commit('runtime/setWindowTitle', { nextTitle: title })
              findTitleFlag = true
            }
            ...
            if (findTitleFlag && findNextFlag) { return }
          }
          page++
        }
      } catch (err) {}
